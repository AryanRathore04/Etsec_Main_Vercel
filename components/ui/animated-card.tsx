...
// Cybersecurity Threat Detection Skeleton
export const ThreatDetectionSkeleton = () => {
  const scale = [1, 1.2, 1];
  ...
  return (
    <div className="p-6 overflow-hidden h-full relative flex items-center justify-center">
      <div className="flex flex-row shrink-0 justify-center items-center gap-4">
        <Container className="h-12 w-12 threat-1">
      ...
      <div className="h-32 w-px absolute top-16 m-auto z-40 bg-gradient-to-b from-transparent via-cyan-500 to-transparent animate-move-1">
        <div className="w-8 h-24 top-1/2 -translate-y-1/2 absolute -left-4">
          <Sparkles />
        </div>
      </div>
    </div>
...
export const AIAnalyticsSkeleton = () => {
  ...
  useEffect(() => {
    // Delay the start of AI animations to match the second card timing
    setTimeout(() => {
      animate(sequence, {
        // @ts-ignore
        repeat: Infinity,
    ...
    }, 5000); // Start when the second card becomes active
  }, []);
  ...
  return (
    <div className="p-6 overflow-hidden h-full relative flex items-center justify-center">
      ...
      <div className="h-32 w-px absolute top-16 m-auto z-40 bg-gradient-to-b from-transparent via-blue-500 to-transparent animate-move-2">
        <div className="w-8 h-24 top-1/2 -translate-y-1/2 absolute -left-4">
          <Sparkles />
        </div>
      </div>
...
// Team Integration Skeleton
export const TeamIntegrationSkeleton = () => {
  const scale = [1, 1.2, 1];
  const transform = ["translateY(0px)", "translateY(-6px)", "translateY(0px)"];
  const sequence = [
    [
  ...
  useEffect(() => {
    // Delay the start of Team animations to match the third card timing
    setTimeout(() => {
      animate(sequence, {
        // @ts-ignore
        repeat: Infinity,
    ...
    }, 10000); // Start when the third card becomes active
  }, []);
  ...
  return (
    <div className="p-6 overflow-hidden h-full relative flex items-center justify-center">
      ...
      <div className="h-32 w-px absolute top-16 m-auto z-40 bg-gradient-to-b from-transparent via-green-500 to-transparent animate-move-3">
        <div className="w-8 h-24 top-1/2 -translate-y-1/2 absolute -left-4">
          <Sparkles />
        </div>
      </div>
  ...
  const sparkleData = React.useMemo(() => {
    if (!mounted) {
      ...
      return Array.from({ length: 12 }, (_, i) => ({
        id: i,
        top: (i * 8.33) % 100, // Distribute evenly
        left: (i * 13.7) % 100, // Different distribution
        opacity: 0.5 + (i % 3) * 0.2,
        duration: 4 + (i % 3),
        scale: 1,
      }));
      ...
      {sparkleData.map((sparkle) => (
        <motion.span
          ...
          animate={
            mounted
              ? {
                  top: `calc(${sparkle.top}% + ${randomMove()}px)`,
                  left: `calc(${sparkle.left}% + ${randomMove()}px)`,
